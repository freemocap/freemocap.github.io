 ...
 *   specialization track (Technology / Science / Art) its own PathGroup,
 *   populated from real module content in `freemocap/university`
 *   (verified via the GitHub API and raw file content, not guessed).
 *   The 9 developer/architecture tiles that used to fill Intermediate
 *   (build + reference pages, no tutorial-tier equivalent at all) moved
 *   into the Technology PathGroup rather than getting their own 4th
 *   column, per explicit instruction; Technology is now much longer
 *   than Science or Art, deliberately, rather than force an even split.
 *   The three PathGroups sit inside a PathColumns wrapper so they read
 *   as parallel side-by-side columns, not a third long vertical scroll;
 *   the Technology/Science/Art chips themselves only appear once, on
 *   TierPicker's Advanced box near the top, not repeated again down
 *   here.
 *
 ...
 * own order; the developer tiles inside Technology keep their real
 * `/build/` sidebar order (autogenerated from each page's own
 * `sidebar_position`, not `src/data/sitePages.ts`'s build list, though
 * today they happen to agree) followed by their `/reference/` pages in
 * reference's order; Advanced's non-developer PathGroup tiles are
 * already in the university repo's own module numbering; Get Started's
 * own three already happened to match `/start/`'s order.
 *
            ...
            <PathGroup label="Technology">
              <TileGrid
                tiles={[
                  ...
                  // This site's own developer docs, not university curriculum
                  // content. They don't have a track of their own, so they land
                  // here per explicit instruction rather than getting a 4th
                  // "Developer" column; uneven length vs. Science/Art is fine.
                  // Kept in their real /build/ and /reference/ sidebar order.
                  {
